// lib/calculators/conversions/surface/screens/surface_screen.dart
...
import '../controllers/surface_controller.dart';
...
class _SurfaceScreenState extends State<SurfaceScreen> {
  static const Key _copyButtonKey = ValueKey<String>('surface.copy');
  static const Key _saveButtonKey = ValueKey<String>('surface.save');
  ...
  final FocusNode _keyboardFocusNode = FocusNode(debugLabel: 'surface_keyboard_focus');
  ...
  String _buildSurfaceReport() {
    final state = _controller.state;
    return [
      '${AppLocalizations.of(context).surfaceUnitM2}: ${state.squareMeter}',
      '${AppLocalizations.of(context).surfaceUnitCm2}: ${state.squareCentimeter}',
      '${AppLocalizations.of(context).surfaceUnitHa}: ${state.hectare}',
      '${AppLocalizations.of(context).surfaceUnitAcre}: ${state.acre}',
      '${AppLocalizations.of(context).surfaceUnitFt2}: ${state.squareFoot}',
    ].join('\n');
  }

  void _showSurfaceSnackBar(String message) {
    if (!mounted) return;
  ...
  Future<void> _copySurfaceToClipboard(AppLocalizations l10n) async {
    await Clipboard.setData(ClipboardData(text: _buildSurfaceReport()));
    _showSurfaceSnackBar(l10n.basicHistoryCopied);
  }

  Future<void> _saveSurfaceToFile(AppLocalizations l10n) async {
    final content = _buildSurfaceReport();

    if (!isHistoryFileExportSupported) {
      _showSurfaceSnackBar(l10n.basicHistoryExportUnsupported);
      return;
    }
    ...
    try {
      ...
      if (filePath == null || filePath.isEmpty) {
        _showSurfaceSnackBar(l10n.basicHistoryExportError);
        return;
      }
      _showSurfaceSnackBar(l10n.basicHistoryExported(filePath));
    } catch (_) {
      _showSurfaceSnackBar(l10n.basicHistoryExportError);
    }
  }

  Widget _buildFieldCard(String label, String value, SurfaceScale scale) {
                                                          ...
                                                          child: ConstrainedBox(
                                                            ...
                                                            child: Column(
                                                              ...
                                                              children: [
                                                                Align(
                                                                  ...
                                                                  child: Row(
                                                                    ...
                                                                    children: [
                                                                      IconButton(
                                                                        key: _copyButtonKey,
                                                                        onPressed: () => unawaited(_copySurfaceToClipboard(l10n)),
                                                                      ...
                                                                      IconButton(
                                                                        key: _saveButtonKey,
                                                                        onPressed: () => unawaited(_saveSurfaceToFile(l10n)),
                                                                ...
                                                                _buildFieldCard(
                                                                  l10n.surfaceUnitM2,
                                                                  _controller.state.squareMeter,
                                                                  SurfaceScale.squareMeter,
                                                                ),
                                                                SizedBox(height: isPhone ? 8 : 12),
                                                                _buildFieldCard(
                                                                  l10n.surfaceUnitCm2,
                                                                  _controller.state.squareCentimeter,
                                                                  SurfaceScale.squareCentimeter,
                                                                ),
                                                                SizedBox(height: isPhone ? 8 : 12),
                                                                _buildFieldCard(
                                                                  l10n.surfaceUnitHa,
                                                                  _controller.state.hectare,
                                                                  SurfaceScale.hectare,
                                                                ),
                                                                SizedBox(height: isPhone ? 8 : 12),
                                                                _buildFieldCard(
                                                                  l10n.surfaceUnitAcre,
                                                                  _controller.state.acre,
                                                                  SurfaceScale.acre,
                                                                ),
                                                                SizedBox(height: isPhone ? 8 : 12),
                                                                _buildFieldCard(
                                                                  l10n.surfaceUnitFt2,
                                                                  _controller.state.squareFoot,
                                                                  SurfaceScale.squareFoot,
                                                                ),
                                                              ],
                                                            ),
                                                          ),
